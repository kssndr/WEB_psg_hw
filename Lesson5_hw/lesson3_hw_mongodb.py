...
order_vacancy_name_for_sj = order_vacancy_name
...
while vacancies_counter_hh < quantity:
    next_page_hh = "https://hh.ru/search/vacancy?L_is_autosearch=false&clusters=true&enable_snippets=true&text="
    html_hh = requests.get(next_page_hh + order_vacancy_name + "&page=" + str(page_hh), headers=header).text
    # print(next_page_hh + order_vacancy_name + "&page=" + str(page_hh))
    ...
    if vacancies_counter_hh < quantity and vacancies_counter_hh % 20 == 0:
        page_hh += 1
...
while vacancies_counter_sj < quantity:
    if page_sj == 1:
        next_page_sj = "https://www.superjob.ru/vacancy/search/?keywords="
        # add = ".html"
        add = ""
        html_sj = requests.get(next_page_sj + order_vacancy_name_for_sj + add, headers=header).text
        P = str(next_page_sj + order_vacancy_name_for_sj + add)
        # print(P)
        nn = P
    else:
        next_sight = requests.get(nn, headers=header).text
        parsed_html_sj = bs(next_sight, 'lxml')
        next_page_sj = "https://www.superjob.ru" + parsed_html_sj.find("a", {"class": "icMQ_ _1_Cht _3ze9n f-test-button-dalshe f-test-link-dalshe"})['href']
        nn = next_page_sj.replace("%5D=14&", "%5D=1&")
        html_sj = requests.get(nn, headers=header).text
        # print(nn)

    # https://www.superjob.ru/vakansii/Kouch.html
    # html_sj = requests.get(next_page_sj, headers=header).text
    # print(add)
    ...
    # определение стоит ли дальше делать запрос на новую страницу (если вакансий обработано меньше 20
    # значит они закончились
    if vacancies_counter_sj < quantity and vacancies_counter_sj % 20 == 0:
        page_sj += 1
        time.sleep(randint(1, 2))
    else:
        break
...
# https://www.superjob.ru/vakansii/kouch.html?geo%5Bc%5D%5B0%5D=1&page=2
# https://www.superjob.ru/vakansii/kouch.html?geo%5Bt%5D%5B0%5D=1&page=2